...
export default function Admin() {
  ...
  // Sample kitchen stock data (you can replace this with your actual data)
  const initialKitchenStock = [
    ...
    // Add more kitchen stock data as needed
  ];

  const [kitchenStock, setKitchenStock] = useState(initialKitchenStock);
  ...
  const handleSearch = () => {
    ...
    const foundItems = initialKitchenStock.filter((item) =>
  ...
  const handleSaveClick = (id, updatedIngredientName) => {
    // Perform the save logic for the edited item
    const updatedKitchenStock = kitchenStock.map((item) => {
      if (item.id === id) {
        return { ...item, ingredientName: updatedIngredientName };
      }
      return item;
    });

    setKitchenStock(updatedKitchenStock);
          ...
          <Grid item xs={16}>
            <section className='content-header'>
              ...
              <div className='header-title'>
                <h1>Kitchen Stock Report</h1>
                <small>Kitchen Stock Report</small>
              </div>
            </section>
          </Grid>
        </Grid>
                  ...
                  {editableId === item.id ? (
                    <div>
                      <input
                        ...
                        onChange={(e) => setKitchenStock((prevKitchenStock) =>
                          prevKitchenStock.map((stockItem) =>
                            stockItem.id === item.id
                              ? { ...stockItem, ingredientName: e.target.value }
                              : stockItem
                          )
                        )}
                      />
                      <button onClick={() => handleSaveClick(item.id, item.ingredientName)}>
                        Save
                      </button>
                    </div>
                  ) : (

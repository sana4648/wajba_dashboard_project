...
import {
  ...
  restaurantByOwner,
  updateRestaurant,
} from "../services/restaurant_services";
...
export default function AddRestaurant() {
  ...
  useEffect(() => {
    const getResturant = async () => {
      const { data: resturantData } = await restaurantByOwner(
        location.state.id
      );
      if (resturantData) {
        setRestaurant(resturantData[0]);
      }
    };
    getResturant();
  ...
  const handleSubmit = async (e) => {
    ...
    if (validateForm()) {
      setIsLoading(true);
      const isAvailable = await checkIfRestaurantExists(
        formData.restaurantName.trim()
      );
      if (isAvailable) {
        setIsLoading(false);
        toast.error("Restaurant already exists with this name.");
        return;
      }
      // Upload restaurant image
      const { error: imageError } = await supabase.storage
        .from("Restaurants")
        .upload(restaurant.id, formData.restaurantImage, {
          cacheControl: "3600",
          upsert: false,
        });
      if (imageError) {
                  ...
                  <Grid item xs={12} sx={{ ml: "3em", mr: "3em" }}>
                    ...
                    <form onSubmit={handleSubmit}>
                      ...
                      <div>
                        ...
                        {errors.restaurantContact && (
                          <p className="error">{errors.restaurantContact}</p>
                        )}
                      ...
                      <div>
                        ...
                        {errors.restaurantImage && (
                          <p className="error">{errors.restaurantImage}</p>
                        )}
                      </div>

                      <button type="submit" disabled={isLoading}>
                        {isLoading ? "Loading..." : "Add Restaurant"}
                      </button>
                    </form>
                  </Grid>
                </Grid>
